/*
 * ChaCha20-Poly1305 Decryption Performance Test (CSV Output)
...
void setup() {
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        ...
        for (int r = 0; r < REPS; r++) {
            ...
            // 2. Decrypt (In-place)
            chachaPoly.decrypt(buffer, buffer, msgSize);
            
            // 3. Check Tag (Verify integrity)
            volatile bool valid = chachaPoly.checkTag(expectedTag, TAG_SIZE);
        }
